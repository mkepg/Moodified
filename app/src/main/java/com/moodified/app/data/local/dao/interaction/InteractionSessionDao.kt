package com.moodified.app.data.local.dao.interaction
...
interface InteractionSessionDao {
    ...
    @Query(
        "SELECT * FROM interaction_sessions WHERE startTimeMillis >= :startMillis AND startTimeMillis < :endMillis ORDER BY startTimeMillis ASC",
    )
    fun getSessionsBetween(
        startMillis: Long,
        endMillis: Long,
    ): Flow<List<InteractionSessionEntity>>

    // Sprint 2 addition: One-shot query for flush aggregation
    @Query(
        "SELECT * FROM interaction_sessions WHERE startTimeMillis >= :startMillis AND startTimeMillis < :endMillis ORDER BY startTimeMillis ASC",
    )
    suspend fun getSessionsListBetween(
        startMillis: Long,
        endMillis: Long,
    ): List<InteractionSessionEntity>
    ...
    suspend fun deleteSessionsBetween(
        startMillis: Long,
        endMillis: Long,
    )
}
